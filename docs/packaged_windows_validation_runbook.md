...
3. Confirm a versioned EXE for Dispatcher Core `2.4.3` appears in `dist/` and that older EXEs, if any, were archived to `dist/Old_exe`.
...
3. Confirm the window title reports Dispatcher Core `2.4.3`.
...
5. Confirm the app later offers cleanup of older EXE copies and ignores the running `2.4.3` build itself.
