...
- Downloaded module payloads are stored in the external `the_golden_standard` folder and become active automatically for that module once the module is reloaded.
...
- In the Python/source workflow, stable updates can download the published EXE into local `dist` and launch it for handoff testing.
- In the packaged EXE workflow, the newer EXE downloads beside the current one, launches separately, and the app can later offer cleanup of older local EXEs.
