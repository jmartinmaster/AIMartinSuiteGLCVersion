...
from app.app_identity import DEB_PACKAGE_NAME, LEGACY_EXE_NAME, format_versioned_deb_name, format_versioned_exe_name, load_version_from_main, normalize_version, parse_version
from app.app_platform import get_platform_update_artifact_kind, get_platform_update_artifact_label, is_ubuntu_runtime, is_windows_runtime, open_with_system_default
...
def _detect_remote_info(preferred_url=None):
    remote_url = None
    if preferred_url:
        remote_url = str(preferred_url).strip()
...
def get_local_module_payload_metadata(dispatcher, option):
    ...
    if dispatcher.are_external_module_overrides_enabled():
    ...
    module = dispatcher.loaded_modules.get(option["key"]) or sys.modules.get(f"app.{option['key']}")
...
class UpdateManagerController:
    ...
    def _remote_executable_candidates(self, row):
        versioned_name = row.get("remote_exe_name") or self._stable_artifact_name_for_version(row.get("remote_version"))
        candidates = []
        if self.stable_artifact_kind == "deb":
            if versioned_name:
    ...
    def check_for_updates(self):
        ...
        for entry in self.local_manifest:
            ...
            if remote_compare is None:
                ...
                status = f"{self._stable_artifact_status_label()} update available"
                update_available = True
            else:
                status = "Up to date"
                update_available = False
            ...
            comparison_rows.append({
                **entry,
                ...
                "remote_version": remote_version,
                "remote_exe_name": self._stable_artifact_name_for_version(remote_version) if remote_compare else None,
                "status": status,
                "update_available": update_available,
            })
    ...
    def _download_remote_executable(self, row):
        ...
        raise RuntimeError(f"No packaged {self._stable_artifact_noun()} artifact was found for the remote version.")
