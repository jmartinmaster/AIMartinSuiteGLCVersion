...
class InternalCodeEditorQtController:
    def __init__(self, payload=None, parent=None, dispatcher=None):
        ...
        bundled_path = str(self.payload.get("bundled_app_path") or getattr(dispatcher, "modules_path", ""))
        ...
        ready_message = "Internal Code Editor viewport ready." if self.embedded else "Internal Code Editor Qt window ready."
        self.write_state(status="ready", message=ready_message, dirty=False)
    ...
    def _build_view_payload(self):
        ...
        theme_tokens = dict(getattr(getattr(dispatcher, "view", None), "theme_tokens", {}) or self.payload.get("theme_tokens") or {})
        return {
            "window_title": "Internal Code Editor - Production Logging Center",
            "title": "Internal Code Editor",
            "subtitle": "Inspect and edit Python modules directly inside the shared PyQt6 workspace.",
    ...
    def _create_view(self):
        self.view = InternalCodeEditorQtView(self, self.payload, parent_widget=self.parent)
