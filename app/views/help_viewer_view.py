...
class RoundedNavChip(tk.Canvas):
    ...
    def _render(self):
        ...
        self.create_text(int(self["width"]) // 2, self.height // 2, text=self.text, font=self.font, fill=text_color)
...
class HelpViewerView:
    ...
    def setup_ui(self):
        ...
        tk.Label(hero_copy, text="Help Center", font=("Segoe UI Semibold", 22), fg=self.palette["hero_title"], bg=self.palette["hero_bg"]).pack(anchor=W)
        tk.Label(
            hero_copy,
        ...
        tk.Label(stats_row, text=f"{len(self.doc_index)} bundled references", font=("Segoe UI Semibold", 10), fg=self.palette["hero_badge_fg"], bg=self.palette["hero_badge_bg"], padx=10, pady=4).pack(side=LEFT)
        tk.Label(stats_row, text="Single-page navigation", font=("Segoe UI Semibold", 10), fg=self.palette["hero_badge_muted_fg"], bg=self.palette["hero_badge_muted_bg"], padx=10, pady=4).pack(side=LEFT, padx=(12, 0))

        action_row = tk.Frame(self.hero_panel, bg=self.palette["hero_bg"])
        action_row.grid(row=0, column=1, sticky=NE, padx=(18, 0))
        tb.Button(action_row, text="Open Current File", bootstyle="primary", command=self.controller.open_active_document).pack(anchor=E)
        tb.Button(action_row, text="Open License File", bootstyle=SECONDARY, command=lambda: self.dispatcher.open_help_document("docs/legal/LICENSE.txt")).pack(anchor=E, pady=(8, 0))
        ...
        self.link_canvas = tk.Canvas(nav_shell, height=52, highlightthickness=0, bd=0, bg=self.palette["page_bg"], xscrollincrement=24)
        ...
        for doc_name, doc_path in self.doc_index:
            button = RoundedNavChip(self.link_bar, text=doc_name, command=lambda name=doc_name, path=doc_path: self.controller.show_document(name, path))
        ...
        self.sub_link_canvas = tk.Canvas(self.subnav_shell, height=42, highlightthickness=0, bd=0, bg=self.palette["page_bg"], xscrollincrement=20)
        ...
        tk.Label(header_band, textvariable=self.doc_title_var, font=("Segoe UI Semibold", 18), fg=self.palette["panel_title"], bg=self.palette["panel_bg"]).grid(row=0, column=0, sticky=W)
        tk.Label(header_band, textvariable=self.doc_meta_var, font=("Segoe UI Semibold", 10), fg=self.palette["panel_badge_fg"], bg=self.palette["panel_badge_bg"], padx=10, pady=4).grid(row=0, column=1, sticky=E)
        tk.Label(header_band, textvariable=self.doc_path_var, font=("Segoe UI", 10), fg=self.palette["text_path"], bg=self.palette["panel_bg"]).grid(row=1, column=0, columnspan=2, sticky=W, pady=(6, 0))
    ...
    def on_link_canvas_resize(self, event):
        ...
        self.link_canvas.itemconfigure(self.link_window, width=max(required_width, event.width))
    ...
    def on_sub_link_canvas_resize(self, event):
        ...
        self.sub_link_canvas.itemconfigure(self.sub_link_window, width=max(required_width, event.width))
    ...
    def render_subnav_for_path(self, doc_path):
        ...
        group_name = self.controller.get_doc_group(doc_path)
        ...
        for section_name, section_path in group.get("sections", []):
            button = RoundedNavChip(self.sub_link_bar, text=section_name, compact=True, command=lambda name=section_name, path=section_path: self.controller.show_document(name, path))
    ...
    def show_document(self, doc_name, doc_path, content):
