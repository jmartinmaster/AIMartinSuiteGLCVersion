...
1. Download the latest versioned `.exe` from the `dist/` folder in the repository (e.g., `Production Logging Center_GLC_v2.4.7.exe`).
...
1. Obtain the `.deb` package (e.g., `production-logging-center-glc_2.4.7_amd64.deb`) from `dist/ubuntu/`.
2. Install it:
   ```bash
   sudo dpkg -i production-logging-center-glc_2.4.7_amd64.deb
...
Produces a versioned `.exe` in `dist/` (e.g., `Production Logging Center_GLC_v2.4.7.exe`). The current EXE is kept in `dist/` and older copies are archived to `dist/Old_exe/` (up to 10 kept).
